...
static void handleAdcDmaError(void)
{
    if (!dma_error) return;
   // CDC_Print("DMA Error Detected. Restarting ADC DMA.\r\n");
    HAL_ADC_Stop_DMA(&hadc1);
    if (HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc_values, 4) != HAL_OK) {
     //   CDC_Print("ADC DMA Restart Failed\r\n");
    	Error_Handler();
    }
...
int main(void)
{
  ...
  ssd1306_Init(&hi2c1);
         ...
         ssd1306_SetCursor(0, 10);
             ssd1306_WriteString("BharatPilot ", Font_11x18, White);
